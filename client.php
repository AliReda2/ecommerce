...
if (isset($_POST['addToCart'])) {
    ...
    $cat = $_POST['cat'];
    $quantity = 1;
    $select = mysqli_query($con, "SELECT * FROM cart WHERE name='$name' AND Uid='$Uid' AND image='$image'");
    if (mysqli_num_rows($select) > 0) {
        echo '<script type="text/javascript"> alert("product already in cart") </script>';
    } else {
        $insert = mysqli_query($con, "INSERT INTO cart (name,price,image,quantity,Uid) VALUES('$name','$price','$image','$quantity','$Uid')");
...
<style>
    ...
    .logOut {
        ...
        /* Apply a transition for smoother appearance/disappearance */
    }

    /* Show the divs when hovering over a certain element */
    .signIn:hover .arrow,
    .signIn:hover .logOut {
        display: block;
        opacity: 1;
        /* Make the elements fully opaque when displayed */
    }
</style>
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        var parentContainer = document.querySelector(".signIn");

        parentContainer.addEventListener("mouseenter", function() {
            arrow.classList.remove("hide-after-delay");
            logOut.classList.remove("hide-after-delay");
        });

        parentContainer.addEventListener("mouseleave", function() {
            setTimeout(function() {
                arrow.classList.add("hide-after-delay");
                logOut.classList.add("hide-after-delay");
    ...
    <nav>
        ...
        <div class="signIn">
            ...
            if (isset($_COOKIE["uname"])) {
                echo '<span class="profile" style="cursor:pointer;">' . $_COOKIE["uname"] . '</span>';
            } else {
        ...
        <?php
        $selectP = mysqli_query($con, "SELECT * FROM cart WHERE Uid='$Uid'") or die("query failed");
        $rowsCount = mysqli_num_rows($selectP);
        ?>
        <div class="cart"><a href="cart.php"><i class="fa-solid fa-cart-shopping"></i><span><sup><?php echo $rowsCount ?></sup></span></a></div>
    </nav>
                                ...
                                ?>
                                    ...
                                    <div class="content">
                                        ...
                                        <form action="" method="post">
                                            ...
                                            <input type="hidden" value="<?php echo $row2['name'] ?>" name="pName">
                                            <input type="hidden" value="<?php echo $row2['price'] ?>" name="pPrice">
                                            <input type="hidden" value="<?php echo $row2['image'] ?>" name="pImage">
                                            <input type="hidden" value="<?php echo $categoryName ?>" name="cat">
                                            <input type="submit" class="buy-1" value="add to cart" name="addToCart">
                                        </form>
                                    </div>
